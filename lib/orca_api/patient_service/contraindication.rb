...
module OrcaApi
  class PatientService < Service
    ...
    class Contraindication < Service
      ...
      #   患者番号
      def get(patient_id)
        first_result = call_orca12_patientmodv37_01 patient_id
      ensure
        unlock_orca12_patientmodv37 first_result
      end
      ...
      def update(patient_id, params)
        ...
        call_orca12_patientmodv37_02(params, first_result).tap do |result|
          # 処理成功したらロックの解除は不要なため
      ...
      def call_orca12_patientmodv37_01(patient_id)
        body = wrap({
                      "Request_Number" => "01",
      ...
      def call_orca12_patientmodv37_02(params, previous_result)
        body = wrap({
      ...
      def call_orca12_patientmodv37_99(result)
        body = wrap({
                      "Request_Number" => "99",
                      "Karte_Uid" => orca_api.karte_uid,
      ...
      def unlock_orca12_patientmodv37(result)
        return unless result
        return if result.locked?
        return unless result.respond_to?(:orca_uid)
        call_orca12_patientmodv37_99(result)
      end
